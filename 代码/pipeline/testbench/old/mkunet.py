...
class MKUNetClassifier(nn.Module):
    """
    MK-UNet 分割头输出经 GAP + 线性层得到整图分类 logits，与 ``IP_train.py`` 的
    ``(B, num_classes)`` 约定一致。输入 ``(B, 1, C, H, W)``（与高光谱 DataLoader 一致）。
    """
    ...
    def __init__(
        ...
        **mk_kwargs,
        ...
        super().__init__()
        v = (variant or "M").strip().upper()
        if v == "T":
            self.backbone = MK_UNet_T(
                num_classes=num_classes, in_channels=in_bands, **mk_kwargs
            )
        elif v == "S":
            self.backbone = MK_UNet_S(
                num_classes=num_classes, in_channels=in_bands, **mk_kwargs
            )
        else:
            self.backbone = MK_UNet(
                num_classes=num_classes, in_channels=in_bands, **mk_kwargs
            )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        b1 = x.squeeze(1)
        # 骨干含 5 次 2×2 下采样，空间尺寸需 ≥32；与 IP_train 默认 16×16 patch 对齐时上采样
        _, _, h0, w0 = b1.shape
        min_sp = 32
        if h0 < min_sp or w0 < min_sp:
            b1 = F.interpolate(
                b1,
                size=(max(h0, min_sp), max(w0, min_sp)),
                mode="bilinear",
                align_corners=False,
            )
        outs = self.backbone(b1)
        p4 = outs[0] if isinstance(outs, (list, tuple)) else outs
        vec = self.gap(p4).flatten(1)
        logits = self.fc(vec)
...
def build_tri_branch_net(
    ...
    **kwargs,
    ...
    """
    供 ``IP_train.resolve_tri_branch_builder('mkunet')`` 使用。
    可选 ``mkunet_variant`` / ``variant``：``T`` / ``S`` / ``M``（默认），对应
    ``MK_UNet_T`` / ``MK_UNet_S`` / ``MK_UNet``。
    ``branch_dim``、``dropout`` 与 baseline 接口对齐，此处不使用。
    """
    if sample_x.dim() != 5:
        raise ValueError("sample_x 应为 (B, 1, C, H, W)")
    _, _, c, _, _ = sample_x.shape
    variant = kwargs.pop("mkunet_variant", kwargs.pop("variant", "M"))
    return MKUNetClassifier(in_bands=c, num_classes=num_classes, variant=variant, **kwargs)
