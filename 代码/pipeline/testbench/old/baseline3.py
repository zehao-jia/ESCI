"""
高光谱海面溢油检测 — 1D / 2D / 3D 三分支融合网络（baseline3）

输入与常见 patch 管线一致: (B, 1, C, H, W)，C 为光谱维，H/W 为空间 patch。
- 1D 分支：空间金字塔池化（SPP 风格）：对 (H,W) 做多尺度自适应平均池化与自适应最大池化并联
  （默认尺度 1x1、2x2、4x4），拼接后经各自 MLP 投影并以可学习系数融合，得到谱空摘要向量。
- 2D 分支：轻量三层 U-Net（3 次 MaxPool 下采样 + 转置卷积上采样与跳跃连接），
  与 baseline1 的差异是将 U-Net 中双卷积替换为蛇形卷积块（1xk + kx1 方向卷积融合）。
