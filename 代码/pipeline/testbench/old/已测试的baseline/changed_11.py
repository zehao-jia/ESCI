...
from testbench.LCGA import LCGA

# ---------------------------------------------------------------------------
# 本文件由 baseline11.py 拷贝修改而来；以下为相对 baseline11 的改动记录（便于对照与复盘）。
#
# 1) 新增依赖：from .LCGA import LCGA（局部曲率引导注意力，见 testbench/LCGA.py）。
#
# 2) 删除（原 ViT 分支）：
#    - SpatialViTBlock（nn.MultiheadAttention + MLP）
#    - CoordAttViTBlock（CoordAtt → LayerNorm → SpatialViTBlock → 还原空间维）
#
# 3) 新增（替代 ViT）：
#    - CoordAttLCGABlock：CoordAtt → flatten 为 (B,H*W,C) → LayerNorm → LCGA(x,H,W) → 还原 (B,C,H,W)。
#
# 4) 并联残差块重命名与参数：
#    - SnakeCoordAttViTBlock → SnakeCoordAttLCGABlock（蛇形 SegNeXt 块 + CoordAttLCGABlock）
#    - 原 vit_num_heads / vit_dropout → lcga_num_heads / lcga_drop（语义对齐 LCGA）
#
# 5) LCGA 构造约定（与 U-Net 多尺度特征图兼容）：
#    - idx=0, rs_id=0：走无 cyclic shift 分支，避免依赖固定分辨率注册的 attn_mask。
#    - split_size/shift_size 默认与 LCGA 类一致 [2,4]、[1,2]；必要时可在 CoordAttLCGABlock 构造参数中覆盖。
#
# 6) _double_conv 内嵌模块：SnakeCoordAttViTBlock → SnakeCoordAttLCGABlock（其余 UNet3Layer / UNetClassifier 与 baseline11 一致）。
# ---------------------------------------------------------------------------
...
class SegNeXtSnakeMSCA(nn.Module):
    ...
    def __init__(self, channels: int):
        ...
        self.snake_large = SnakeStripDWConv(channels, kernel_size=13)
...
class CoordAttLCGABlock(nn.Module):
    """
    坐标注意力 → token 序列 → LCGA（局部曲率引导注意力）→ 还原特征图。
    （对照 baseline11：此处原为 CoordAttViTBlock + SpatialViTBlock。）
    ...
    def __init__(
        ...
        sp = split_size if split_size is not None else [2, 4]
        sh = shift_size if shift_size is not None else [1, 2]
        self.lcga = LCGA(
            dim=ch,
            num_heads=num_heads,
            split_size=sp,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        tokens = self.lcga(tokens, h, w)
        return tokens.transpose(1, 2).reshape(b, c, h, w)


class SnakeCoordAttLCGABlock(nn.Module):
    """
    蛇形卷积与 CoordAtt→LCGA 的并联加残差块。
    结构：shortcut + snake(x) + coord_att_lcga(x)
    （对照 baseline11：原名 SnakeCoordAttViTBlock，第三支为 CoordAtt→ViT。）
    """
    ...
    def __init__(
        ...
        ch: int,
        reduction: int = 16,
        lcga_num_heads: int = 4,
        lcga_drop: float = 0.1,
    ):
        super().__init__()
        self.snake = SegNeXtSnakeEncoderBlock(ch)
        self.coord_att_lcga = CoordAttLCGABlock(
            ch,
            reduction=reduction,
            num_heads=lcga_num_heads,
            drop=lcga_drop,
        )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return x + self.snake(x) + self.coord_att_lcga(x)


def _double_conv(in_ch: int, out_ch: int) -> nn.Sequential:
    # 最后一层原为 SnakeCoordAttViTBlock，已换为 SnakeCoordAttLCGABlock（见文件头改动说明）。
    return nn.Sequential(
        nn.Conv2d(in_ch, out_ch, 3, padding=1, bias=False),
        nn.BatchNorm2d(out_ch),
        nn.ReLU(inplace=True),
        SnakeCoordAttLCGABlock(out_ch),
    )
