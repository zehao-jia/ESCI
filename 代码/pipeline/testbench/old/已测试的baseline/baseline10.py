...
'''
这是baseline10,在baseline9的基础上，将 _double_conv 中的第二层 2D 卷积替换为：
蛇形卷积块（SnakeConvUnit）和坐标注意力→ViT块（CoordAttViTBlock）的并联加残差结构（SnakeCoordAttViTBlock）。
流程：输入 → Conv2d → BN → ReLU → [蛇形卷积分支 || CoordAtt → token化 → ViT×2 → 还原] → 融合
输入为(B, 1, C, H, W)，输出为(B, num_classes)，其中num_classes默认为2。
'''
...
class CoordAttViTBlock(nn.Module):
    """
    坐标注意力作用:降噪,增强空间特征表达。
    CoordAtt → token化 → 2个 ViT Block → 还原特征图。
...
class SnakeCoordAttViTBlock(nn.Module):
    ...
    def __init__(self, ch: int, snake_kernel_size: int = 3, reduction: int = 16,
                 vit_num_heads: int = 4, vit_dropout: float = 0.1):#卷积核的kernel size为3
        super().__init__()
