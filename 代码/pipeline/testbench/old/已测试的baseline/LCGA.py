# 局部曲率引导注意力模块（Local Curvature-guided Attention, LCGA）
# 核心设计：基于“窗口划分+曲率特征提取+门控注意力”的流程，
# 通过局部窗口内的特征曲率计算引导注意力权重分配，结合门控机制动态平衡局部细节与全局关联，
# 强化特征的结构依赖性与局部辨识度，在低计算成本下提升特征表达的精准性
...
class LCGA(nn.Module):
    ...
    局部曲率引导注意力模块（Local Curvature-guided Attention, LCGA）
    功能：局部窗口内计算特征曲率，引导注意力权重分配，强化结构依赖
    核心设计：
        - 局部窗口划分：降低计算成本，聚焦局部结构曲率
        - 曲率特征提取：基于窗口内特征梯度计算曲率，捕捉结构变化
        - 门控注意力融合：动态平衡曲率引导特征与原始特征，提升表达精准性
        - QKV轻量化生成：1×1卷积投影，避免复杂维度变换
    Args:
        dim: 输入/输出通道数
        window_size: 局部窗口尺寸（默认(4,4)）
        num_heads: 注意力头数（默认4）
