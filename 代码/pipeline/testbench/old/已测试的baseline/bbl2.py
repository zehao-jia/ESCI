...
'''
此中先验来自**原图全波段** patch 的通道均值（非 PCA），
经 Sobel 边缘检测增强后投影到各层特征通道数，
指导注意力机制关注边界信息；UNet 主干仍使用 PCA 输入。
Attention头数:8
base=64
'''
...
class UNet3Layer(nn.Module):
    ...
    def __init__(self, in_channels: int, origin_channels: int = 0, base: int = 64):
        super().__init__()
        ...
        # prior 输入通道: Sobel 边缘(1) + 原图波段(origin_channels)
        prior_in = 1 + max(origin_channels, 1)
        self.prior_conv1 = nn.Sequential(
            nn.Conv2d(prior_in, c1, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(c1), nn.ReLU(inplace=True))
        self.prior_conv2 = nn.Sequential(
            nn.Conv2d(prior_in, c2, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(c2), nn.ReLU(inplace=True))
        self.prior_conv3 = nn.Sequential(
            nn.Conv2d(prior_in, c3, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(c3), nn.ReLU(inplace=True))

    def forward(self, x: torch.Tensor, x_origin: torch.Tensor = None) -> torch.Tensor:
        B, C, H, W = x.shape
        prior_src = x_origin if x_origin is not None else x
        pri = torch.mean(prior_src, dim=1, keepdim=True)             # (B,1,H,W) 原图波段均值
        pri_edge = self.boundary(pri)                                # (B,1,H,W) Sobel 边缘增强
        pri = torch.cat([pri_edge, x_origin], dim=1)                 # 融合边界信息
...
class UNetClassifier(nn.Module):
    uses_origin_prior = True

    def __init__(self, in_bands: int, num_classes: int = 2, origin_channels: int = 0):
        super().__init__()
        self.unet = UNet3Layer(in_channels=in_bands, origin_channels=origin_channels, base=64)
        self.gap = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Linear(2, num_classes)

    def forward(self, x: torch.Tensor, x_origin: torch.Tensor = None) -> torch.Tensor:
        b1 = x.squeeze(1)
        x_orig = x_origin.squeeze(1) if x_origin is not None else None
        seg_logits = self.unet(b1, x_origin=x_orig)
...
def build_tri_branch_net(
    ...
    branch_dim: int = 128,
    dropout: float = 0.4,
    origin_channels: int = 0,
    **kwargs,
) -> UNetClassifier:
    if sample_x.dim() != 5:
        raise ValueError("sample_x 应为 (B, 1, C, H, W)")
    _, _, c, h, w = sample_x.shape
    return UNetClassifier(in_bands=c, num_classes=num_classes, origin_channels=origin_channels)
...
if __name__ == "__main__":
    ...
    x_origin = torch.randn(B, 1, C_orig, H, W)
    net = UNetClassifier(in_bands=C, num_classes=2)
    y = net(x, x_origin=x_origin)
    print("PCA 输入:", tuple(x.shape), "原图先验:", tuple(x_origin.shape), "输出:", tuple(y.shape))
