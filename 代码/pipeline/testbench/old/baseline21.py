...
'''
baseline17：在 baseline16 基础上，将先验图改为平均结果经 Sobel 算子处理后的结果。
baseline21:在17的基础上改每层头的数量
'''
...
class UNet3Layer(nn.Module):
    def __init__(self, in_channels: int, base: int = 16):
        ...
        self.sobel_prior = SobelPrior()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, C, H, W = x.shape
        pri = self.sobel_prior(x)
        e1 = self.enc1(x, pri)
