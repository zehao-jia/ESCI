...
class DoubleConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        # Transformer注意力替换top_k attention
        self.topk_attention = TransformerAttention(
            in_channels=out_channels,
            input_resolution=32,  # 默认分辨率，会根据实际输入自动调整
            num_heads=out_channels // 32,  # 每个head 32维
            win_size=8,
            shift_size=0,
            mlp_ratio=4.,
            token_mlp='leff'
...
class UNet_Deep(nn.Module):
    ...
    def __init__(self, in_channels=25, out_channels=25, num_filters=64):
        ...
        # Transformer模块（替换Top-K Token Attention模块）
        self.topk_attention5 = TransformerAttention(
            in_channels=num_filters * 16,
            num_heads=(num_filters * 16) // 32,
            win_size=8,
            token_mlp='leff'
        )
        self.topk_attention4 = TransformerAttention(
            in_channels=num_filters * 8,
            num_heads=(num_filters * 8) // 32,
            win_size=8,
            token_mlp='leff'
        )
        self.topk_attention3 = TransformerAttention(
            in_channels=num_filters * 4,
            num_heads=(num_filters * 4) // 32,
            win_size=8,
            token_mlp='leff'
        )
        self.topk_attention2 = TransformerAttention(
            in_channels=num_filters * 2,
            num_heads=(num_filters * 2) // 32,
            win_size=8,
            token_mlp='leff'
        )
        self.topk_attention1 = TransformerAttention(
            in_channels=num_filters,
            num_heads=num_filters // 32,
            win_size=8,
            token_mlp='leff'
        )
