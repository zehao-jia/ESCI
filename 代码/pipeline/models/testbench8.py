...
class WKVFunction(torch.autograd.Function):
    """
    WKV 递归前向/反向计算（8 头独立），使用 cumsum 批量化实现（无 for 循环）。
    
    等价于原始递推：
        a[t] = a[t-1]*exp(-w) + exp(k[t])*v[t]   (初始 a[-1]=0)
        b[t] = b[t-1]*exp(-w) + exp(k[t])         (初始 b[-1]=0)
        wkv[t] = (exp(u+k[t])*v[t] + a[t-1]) / (exp(u+k[t]) + b[t-1])
    
    批量化展开：
        a[t] = sum_{i=0}^{t} exp(k[i])*v[i] * exp(-w*(t-i))
             = exp(-w*t) * cumsum_{i=0..t} exp(k[i])*v[i] * exp(w*i)
    """
    @staticmethod
    def forward(ctx, w, u, k, v):
        ...
        t_idx = torch.arange(T, device=k.device, dtype=torch.float).view(1, T, 1, 1)
        exp_w_pow = torch.exp(w.float().view(1, 1, H, 1) * t_idx)

        exp_k = torch.exp(k.float())
        exp_kv = exp_k * v.float()

        cum_a = torch.cumsum(exp_kv * exp_w_pow, dim=1)
        cum_b = torch.cumsum(exp_k * exp_w_pow, dim=1)

        a_full = cum_a / exp_w_pow
        b_full = cum_b / exp_w_pow

        a_prev = torch.zeros_like(a_full)
        a_prev[:, 1:] = a_full[:, :T-1]
        b_prev = torch.zeros_like(b_full)
        b_prev[:, 1:] = b_full[:, :T-1]

        exp_u_k = torch.exp(u.float().view(1, 1, H, 1) + k.float())
        y = (exp_u_k * v.float() + a_prev) / (exp_u_k + b_prev + 1e-8)
    ...
    def backward(ctx, gy):
        ...
        with torch.enable_grad():
            ...
            y_f = WKVFunction.forward(ctx, w_f, u_f, k_f, v_f)
