...
TRAIN_TEST_SPLIT_MODE = 'balanced_per_class'
...
def create_data_loader(i):
    ...
    if cache_key in _data_loader_cache:
        ...
        y_all = cached["y_all"]
        y = cached["y"]
        priori = cached["priori"]
    else:
        print(f"  [预处理] 加载 GM0{i}.mat ...", flush=True)
        X, y = loadData(i)
        priori = np.mean(X, axis=-1)
        ...
        print(
            f"  [预处理] patch {X_patches.shape} ({time.perf_counter() - t_patch:.1f}s)",
            flush=True,
        )
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                outputs = net(data)
                # 以下保留 moe_aux 仅供 TensorBoard 日志记录，不再加入 loss
...
if __name__ == '__main__':
    ...
    TRAIN_MODELS = [
    "baseline2",
    "baseline_unet2layer_ag",
    "baseline_unet2layer_moe_deform",
    "baseline_unet2layer_moe_fixed"
    ]
