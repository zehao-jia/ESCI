...
def loadData():
    # 读入数据
    data = sio.loadmat('../data/GM01.mat')['img']  #读入高光谱数据
    labels = sio.loadmat('../data/GM01.mat')['map']  #   读入标签数据
...
if __name__ == '__main__':
    ...
    #写入报告（新增召回率和AUC）
    file_name = "cls_result/classification_report3.txt"
    with open(file_name, 'w') as x_file:
        ...
        # 新增：每类准确率、召回率、AUC
        x_file.write('\nPer-class Metrics:\n')
        x_file.write('Class\tAccuracy(%)\tRecall(%)\tAUC(%)\n')
        target_names = ['background', 'oil']
        for i in range(len(target_names)):
            x_file.write('{}\t\t{:.4f}\t\t{:.4f}\t\t{:.4f}\n'.format(
                target_names[i], each_acc[i],
            ))
