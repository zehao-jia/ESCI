...
def loadData():
    # 读入数据
    data = sio.loadmat('../data/Indian_pines_corrected.mat')['indian_pines_corrected']  #读入高光谱数据
    labels = sio.loadmat('../data/Indian_pines_gt.mat')['indian_pines_gt']  #   读入标签数据
...
def test(device, net, test_loader):
    ...
    for inputs, labels in test_loader:
        ...
        outputs = net(inputs)
        outputs = np.argmax(outputs.detach().cpu().numpy(), axis=1)#取概率最大的类别
        #拼接所有批次的预测和真实标签
        if count == 0:#首次迭代时,y_pred_test直接赋值为当前结果
            y_pred_test = outputs
            y_test = labels
            count = 1
        else:
            y_pred_test = np.concatenate((y_pred_test, outputs))#随后将新得到的结果拼接
            y_test = np.concatenate((y_test, labels))

    return y_pred_test, y_test#返回所有测试样本的预测类别和真实类别
...
if __name__ == '__main__':
    ...
    y_pred_test, y_test = test(device, net, test_loader)#测试模型
